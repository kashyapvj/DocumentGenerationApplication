...
namespace DocumentGenerationApplication.Controllers
{
    ...
    public class SalaryController : Controller
    {
        ...
        //    try
        //    {
        //        var bands = _repository.GetAllBands();
        //        return Json(bands);
        //    }
        //    catch (Exception ex)
        //    {
        ...
        //        //TotalFixedComponent => Basic + HRA + StatutoryBonus + NPS + VPF + RFB + SpecialAllowance;
        //        bool isEligibleForESIC = (model.IsMetro && inputModel.TotalCTC < 254000) || (!model.IsMetro && inputModel.TotalCTC < 295250);
        //        bool isUnderWageLimit = model.TotalFixedMonthlyComponent < 21000;
        //        if (isEligibleForESIC && isUnderWageLimit)
        //        {
        ...
        //        //TotalDeductions => PFEmployee + ESICEmployee + ProfessionalTax;
        //        model.TotalDeductions = model.PFEmployee + model.ESICEmployee + model.ProfessionalTax;

        //        var _salaryBreakdown = new FillPdfTemplateInput();
        //        _salaryBreakdown.salaryValues = model;
        //        _salaryBreakdown.employeeDetails = inputModel;

        //        // Save in repo (this is where duplicates, etc. will throw)
        //        await _repository.SaveEmployeeDetailsAsync(_salaryBreakdown);

        //              // Generate PDF on success only
        //        var pdfBytes = GeneratePdfFromWord(_salaryBreakdown);
        //        if (pdfBytes == null || pdfBytes.Length == 0)
        //            return StatusCode(500, new { message = "PDF generation failed." });

        //        // Return the PDF (fetch will catch it and download)
        ...
        //        {
        //            // ❌ Already in system
        //            return Json(new { success = false, message = "❌ " + result.Message, data = result });
        //        }
        //        else
        //        {
        //            // ✅ Available
        //            return Json(new { success = true, message = "✅ Email is available!", data = result });
        //        }
        //    }
        //    catch (Exception ex)
        //    {
